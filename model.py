...
def preprocess_image_file_train(line_data):
    ...
    path_file = line_data['filename'][0].strip()
    '''
...
### Loading CSV data
#core_path = '~/Documents/final_yp/simulation/uda_data/'
csv_path = 'train.csv'
data_files_s = pd.read_csv(csv_path,
                         index_col = False)
#data_files_s['direction'] = pd.Series('s', index=data_files_s.index)
data_files_s.columns = ['filename','bull','angle']
### Smoothing steering data
rev_steer_s = np.array(data_files_s.angle,dtype=np.float32)


t_s = np.arange(len(rev_steer_s))
x_s = np.array(data_files_s.angle)
y_s = rev_steer_s
...
print(data_files_s['filename'][0].strip())
image_c = process_newimage_file(data_files_s['filename'][0].strip())
...
for i_pr in range(50):
    ...
    history = model.fit_generator(train_r_generator,
            steps_per_epoch=20224//batch_size, nb_epoch=1,validation_data=valid_s_generator,
